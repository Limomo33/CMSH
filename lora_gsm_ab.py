...
class FusionGate(nn.Module):
    ...
    def forward(self, feat1, feat2):
        ...
        transformed = F.relu(self.transform(combined))
        return g * feat1 + (1-g) * transformed

# 在模型中引入
self.fusion_gate = FusionGate(64)
...
class TransformerModel(nn.Module):
    ...
    def forward(self, pep, mhc, adj, mhc_seq, pep_seq):
        ...
        pooled1 = torch.mean(subgraph_features1, dim=1)
        pooled2 = torch.mean(subgraph_features2, dim=1)
        combined = self.FusionGate([pooled1, pooled2], dim=1)
...
#         merge_2 = torch.cat([fc2, Y_2], dim=1)
        ...
        # fc3 = self.linear6(Y_2)  # merge_1
        # fc3 = F.leaky_relu(fc3)
        # fc3 = self.dropout(fc3)

        out = self.linea7(combined)
...
def cross_validation_training_transformer_gat(training_data, test_dict, global_args):
    ...
    optimizer = optim.AdamW([
        {'params': model.parameters(), 'lr': 0.00001, 'weight_decay': 0.0001}
    ])
    ...
    for epoch in range(num_epochs):
        ...
        foutput(
            str(epoch) + "\t"  + str(avg_train_loss) + "\t" + str(avg_val_loss) + "\t" + str(avg_val_pcc) + "\t" + str(
                avg_val_roc) + "\t" + str(avg_val_acc)+ "\t" + str(avg_val_srcc), output_path + 'result.txt')
        plt.clf()
