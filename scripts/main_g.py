...
def read_validation_data_esm(path, pseq_dict, global_args):
    ...
    tokenizer = AutoTokenizer.from_pretrained(new_dir)
    # c=[]
    pdb_dir = '/root/adjacency_matrices2.txt'
    allele_matrices = load_adjacency_matrices(pdb_dir)    
    f = open(path, "r")
    for line in f:
        info = re.split("\t", line)  # Retrive information from a tab-delimited line
        allele = info[1]
        if allele in pseq_dict.keys():
            adj = get_adjacency_matrix(allele, allele_matrices)
            ...
            for residue_index in range(12):
                ...
                if 12 - residue_index > len(pep):
                    ...
                    pep_blosum.append(blosum_matrix[aa[pep[len(pep) - 12 + residue_index]]])
            new_data = [pep_blosum, pseq_dict[allele][0], pseq_dict[allele][1],adj, pseq_dict[allele][2],tokenizer(
                pep,
                max_length=12,
                padding="max_length",
                truncation=True,
...
for split in range(n_splits):
    ...
    models.append(cross_validation_training_transformer_gat(training_data, test_dicts,
                                                             global_args))
